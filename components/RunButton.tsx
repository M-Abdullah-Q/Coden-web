...
const RunButton = () => {
  const { code, languageId, boilerplates, language, fullBoilerplates } =
    useCodeContext();
  ...
  const handleRun = async () => {
    ...
    const subCode =
      boilerplates?.[language].replace("###USER_CODE_HERE###", code ?? "") ||
      "";
    console.log(subCode);
    ...
    try {
      const res = await axios.post(`${NEXT_PUBLIC_BASE_URL}/api/submission`, {
        subCode,
        languageId,
        tests,
        timeLimit,
