  ...
  useEffect(() => {
    async function sse() {
      ...
      const eventSource = new EventSource(
        `${baseUrl}/api/result?tokens=${tokens}`
      );
      ...
      {result ? (
        isError ? (
          <div className="flex justify-center items-center h-full min-h-[300px]">
            No Test Results to be displayed
          </div>
        ) : (
          <Tabs defaultValue="test-0" className="h-[250px] ">
            <div className="relative">
              <div
                className="overflow-x-auto whitespace-nowrap rounded-lg"
                style={{
                ...
                <TabsList className="flex space-x-2 min-w-max">
                  {tests.map((_, index) => (
                    <TabsTrigger
                      ...
                      style={{
                        backgroundColor:
                          results?.statuses?.[index] === 3 ? "green" : "red",
            ...
            <div
              className="mt-4 max-h-[180px] overflow-y-auto whitespace-nowrap rounded-lg"
              style={{
                WebkitOverflowScrolling: "touch",
                msOverflowStyle: "none",
                scrollbarWidth: "none",
                ...
                >
                  <div className="p-4 rounded-md bg-muted border border-border">
                    ...
                    <p className="font-semibold text-sm mt-2">
                      {results?.statuses?.[index] && results.statuses[index] > 4
                        ? "Error"
                        : "Your output"}
                    </p>
                    <textarea
                      ...
                      readOnly
                      value={
                        results?.statuses?.[index] !== undefined &&
                        results.statuses[index] > 4
                          ? execErrors?.[results.statuses[index]]
                              ?.description ?? "Unknown error"
                          : results?.stdout?.[index] ?? "null"
                      }
                    ></textarea>
                    <p className="font-semibold text-sm mt-2">
                      Expected Output:
